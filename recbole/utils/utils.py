...
def meta_minibatch(meta_train_data, batch_size):
    # dict {task: instances}
    support, query = meta_train_data
    max_instance_num = 0
    for t in support:
        if support[t].shape[0] > max_instance_num:
            max_instance_num = support[t].shape[0]

    for i in range(0, max_instance_num, batch_size):
        batch_support, batch_query = dict(), dict()
        for t in support:
            instance_num = support[t].shape[0]
            if i + batch_size < instance_num:
                batch_support[t] = support[t][i:i + batch_size]
            elif i < instance_num:
                batch_support[t] = support[t][i:]
        for t in query:
            instance_num = query[t].shape[0]
            if i + batch_size < instance_num:
                batch_query[t] = query[t][i:i + batch_size]
            elif i < instance_num:
                batch_query[t] = query[t][i:]
