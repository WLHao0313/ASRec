...
class Trainer(AbstractTrainer):
    ...
    def _valid_epoch(self, valid_data, show_progress=False):
        ...
        Returns:
            ...
            dict: valid result
        """
        valid_result,_ = self.evaluate(valid_data,Test=False, load_best_model=False, show_progress=show_progress)
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                inference_time = valid_end_time - valid_start_time
    ...
    def _full_sort_batch_eval(self, batched_data,Test):
        interaction, history_index, positive_u, positive_i = batched_data
        try:
            # Note: interaction without item ids
            if Test:
                scores,test_items_emb = self.model.full_sort_predict(interaction.to(self.device))
            else:
                scores, test_items_emb = self.model.full_sort_predict_valid(interaction.to(self.device))
        ...
        return interaction, scores, positive_u, positive_i,test_items_emb
    ...
    @torch.no_grad()
    def evaluate(self, eval_data,Test=False, load_best_model=True, model_file=None, show_progress=False):
        r"""Evaluate the model based on the eval data.
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            if Test==False:
                interaction, scores, positive_u, positive_i,test_items_emb = eval_func(batched_data,Test=False)
            else:
                interaction, scores, positive_u, positive_i, test_items_emb = eval_func(batched_data, Test=True)
            test_feature.append(test_items_emb)
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
