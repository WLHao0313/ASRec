...
def save_split_dataloaders(config, dataloaders):
    ...
    # 添加保存成功提示
    logger.info(set_color('Successfully save split data','pink'))
...
def sequential_augmentation(dataset):
    ...
    if dataset.dataset_name=='yelp':
        new_data = {dataset.uid_field: [],
                    dataset.iid_field: [],
                    dataset.item_list_length_field: [],
                    dataset.business_id_list_field: [], }
        for i in range(len(old_data)):
            seq = old_data[dataset.business_id_list_field][i]
            ...
            new_data[dataset.business_id_list_field].append(seq)
            seq = seq[:seq_len]
            for end_point in range(1, seq_len):
                ...
                new_data[dataset.business_id_list_field].append(new_seq)

        new_data[dataset.business_id_list_field] = torch.stack(new_data[dataset.business_id_list_field], dim=0)
        new_data[dataset.item_list_length_field] = torch.tensor(new_data[dataset.item_list_length_field],
                                                                dtype=torch.long)
...
def create_samplers(config, dataset, built_datasets):
    ...
    print('-----------------')
    print(train_neg_sample_args)
    print('-----------------')

    if train_neg_sample_args['strategy'] != 'none':
    # if train_neg_sample_args['sample_num'] != 'none':
