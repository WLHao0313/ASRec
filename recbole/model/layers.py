...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps, max_seq_length):
        ...
        self.conv_middle = nn.Conv1d(max_seq_length, max_seq_length, kernel_size=5, padding=2)
        ...
        self.contra_scale1 = nn.Parameter(torch.randn(1, max_seq_length, 1))
        self.contra_scale2 = nn.Parameter(torch.randn(1, max_seq_length, 1))
        self.contra_scale3 = nn.Parameter(torch.randn(1, max_seq_length, 1))
        # 门控机制的参数
        self.gate_size = hidden_size // n_heads * 2
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        gate_inputs = torch.cat([value_layer, context_layer], dim=-1)
        gate_activations = F.linear(gate_inputs, self.w_gate, self.b_gate)
        reset_gate, input_gate = gate_activations.chunk(2, dim=-1)

        reset_gate = torch.sigmoid(reset_gate)
        input_gate = torch.sigmoid(input_gate)

        new_context_layer = torch.nn.functional.gelu(context_layer + reset_gate * value_layer)
        context_layer = (1 - input_gate) * context_layer + input_gate * new_context_layer
        ...
        sim = torch.einsum("bld,bld->bld", xn, xn)
        sim = nn.functional.softmax(sim, dim=2) + nn.functional.softmax(sim, dim=1)
        V_neg = torch.einsum("bld,bld->bld", sim, x)
        V = self.orgin_scale * x - self.contra_scale * V_neg

        context_layer = torch.nn.functional.gelu(V)
...
class AHFF(nn.Module):
    def __init__(self, hidden_size, layer_norm_eps):
        ...
        self.threshold_param = nn.Parameter(torch.tensor(0.8))
    ...
    def forward(self, x_in):
        ...
        high_threshold = torch.quantile(normalized_energy, self.threshold_param)
        dominant_frequencies = normalized_energy > high_threshold
        freq_mask = torch.zeros_like(x_fft, device=x_fft.device)
        freq_mask[dominant_frequencies] = 1
        x_masked = x_fft * freq_mask.to(x.device)
        x_weighted = x_masked
        x_weighted = torch.fft.ifft(x_weighted, n=N, dim=1)
        x = x_weighted.to(dtype)
        x = x.view(B, N, C)
        hidden_states = self.out_dropout(x)
        hidden_states = self.LayerNorm(hidden_states+x_org)
        return hidden_states
